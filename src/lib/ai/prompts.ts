...
**输出格式（每行一个 JSON，所有值必须用英文，严禁中文）：**
{"index": N, "relevant": true/false, "content_type": "paper"|"model-product"|"institutional-news"|"investment-news", "category": "AI4S"|"AI4SS"|"AI4R", "region": "cn"|"western"|"global", "ai_role": "core-method"|"tool-application", "reason": "简短（20字内）"}

**ai_role 字段（核心区分）：**
- **core-method**：论文的核心贡献在于 AI 方法/模型/框架本身的突破，目的是推动科学发现范式变革。例如：提出一个新的图神经网络架构用于分子性质预测、设计了一种新的蛋白质设计算法、开创性地将强化学习用于实验自动化。收录优先级最高。
- **tool-application**：论文仅将现有的、成熟的 AI 技术作为工具来回答一个具体的科学问题（如"用 CNN 分类星体图像""用 LSTM 预测天气"）。AI 部分没有方法论创新，科学贡献在应用领域。文献稀少时补位收录，充足时可能被筛掉。

**判断标准：**
- 问自己：如果去掉 AI 部分，这篇论文还有没有独立的科学贡献？有 → tool-application；没有 → core-method
- 如果论文发表在 AI 顶会（NeurIPS/ICML/ICLR），通常属于 core-method
- 如果论文发表在传统科学期刊且只把 AI 当工具用，属于 tool-application
- 非论文类型（模型产品、机构动态、产业投资）ai_role 统一填 "core-method"`;
